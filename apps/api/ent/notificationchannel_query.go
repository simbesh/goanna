...
func (_q *NotificationChannelQuery) QueryNotificationEvents() *NotificationEventQuery {
...
func (_q *NotificationChannelQuery) WithNotificationEvents(opts ...func(*NotificationEventQuery)) *NotificationChannelQuery {
	query := (&NotificationEventClient{config: _q.config}).Query()
...
func (_q *NotificationChannelQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*NotificationChannel, error) {
	...
	if query := _q.withNotificationEvents; query != nil {
		if err := _q.loadNotificationEvents(ctx, query, nodes,
			func(n *NotificationChannel) { n.Edges.NotificationEvents = []*NotificationEvent{} },
			func(n *NotificationChannel, e *NotificationEvent) {
...
func (_q *NotificationChannelQuery) loadNotificationEvents(ctx context.Context, query *NotificationEventQuery, nodes []*NotificationChannel, init func(*NotificationChannel), assign func(*NotificationChannel, *NotificationEvent)) error {
	...
	query.Where(predicate.NotificationEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(notificationchannel.NotificationEventsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.notification_channel_notification_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "notification_channel_notification_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "notification_channel_notification_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
